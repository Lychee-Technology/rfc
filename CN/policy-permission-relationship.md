...
Policy 独立存在。它**不依赖** permission 即可存在，也**不引用** permission 记录。
...
`permissions` claim 的存在**不使** `permission_profile` 成为 canonical，authorizer 消费 `permissions` 也**不使** legacy permission 模型成为新 API 设计的依据。
