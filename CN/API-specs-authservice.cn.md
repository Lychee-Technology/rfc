...
`login/{provider}`、`id_bindings/{provider}`、`auth/revoke` 使用基于 body 的项目解析，顺序为：请求 body → authorizer claim（仅 login）→ 配置默认值；任何显式提供的值都必须是合法 UUID 且等于配置的默认值，否则请求被拒（`invalid_project_id` / `invalid_project_scope`）。

`auth/profile/{user_id}` 与 `auth/refresh` **不**执行该校验：它们直接使用 authorizer JWT 中的 `project_id` claim（profile：`handler_profile.go`；refresh：`handler.go` / `service.go`）。由于 LTBase 的 access/refresh token 本就只为部署项目签发，在单项目部署中有效作用域一致；但 handler 只是信任该 claim，不会再把它与 `PROJECT_ID` 比对。见 §5.4 与 §5.6。
...
服务的签名公钥**不由任何路由提供**。此前的 `GET /auth/jwks.json` 路由已被移除，现在返回 `404 not_found`。JWKS 文档作为静态产物发布，托管在 `AUTH_JWKS_URL` 配置的 URL 上；服务在 refresh 时会拉取该 URL，用于校验自己此前签发的 access token。

该文档是带顶层 `keys` 数组的标准 JWKS（由 `signer.go` 的 `buildRSAJWKS` 生成；`access_token_verify.go` 中的 refresh 校验器要求这一格式）：
...
`{provider}` 路由在路由 manifest 中带 `expand: provider` 标记（`routemanifest.ExpandProvider`）。部署只为 `EnableLogin` / `EnableIDBinding` 标志为真的 provider 展开出具体的 `POST /api/v1/login/<provider>` 与 `POST /api/v1/id_bindings/<provider>` 路由；标志为关的已配置 provider 不会生成路由。每条 provider 路由由该 provider 自己的 IdP authorizer 前置。provider 路径值会被转为小写，且必须在 `AUTH_PROVIDERS` 允许列表内。
...
与网关的过期校验交互：由于 `auth/refresh` 前置 `LTBaseRefresh` JWT authorizer（会校验 refresh JWT 的 `exp`），已过期的 refresh token 会在 Lambda 运行之前于网关处以 `401` 拒绝。因此 Lambda 自身的 `refresh_expired` 路径属于纵深防御，只在绕过网关的场景下才会触发（例如直接 invoke，或某个不强制校验 `exp` 的 authorizer）；经网关前置的客户端通常不会命中它。见 §5.4。
...
响应（`200 OK`）：形状与 login 相同，含 `access_token`、`refresh_token`、`api_base_url`。
...
提供的 `access_token` 会对照 `AUTH_JWKS_URL` 的 JWKS 校验（`access_token_verify.go`），但校验范围是刻意收窄的：只校验 RS256 签名（按 `kid` 选择密钥）、`token_use` 是否为 `access`、`iss` 是否等于配置的 issuer。标准 claim 校验被关闭：过期时间（`exp`）、`aud`、`sub`、`project_id`、`jti` 都**不**校验，且该 token 不与 refresh token 的 subject/project 绑定。跳过 `exp` 是有意为之（access token 过期后刷新是正常场景）；其余省略意味着此检查只能证明呈递了*某个*来自该 issuer 的 access token，不能证明它属于同一 session。校验失败 → `401 access_invalid`。
...
用途：查询公开 profile，范围限定在调用方所属项目内。项目内任意已认证调用方都可读取。

Authorizer：`LTBase`（access JWT）。消费的 claim：`project_id`（必需 → 否则 `401 auth_required`）。查询直接以该 claim 的 `project_id` 作为作用域，不与 `PROJECT_ID` 比较（见 §2.2）。
...
- `aaa.md`：权威的 AAA 模型，含身份模型、确定性 user ID、login/binding 时序、JWT 设计。
