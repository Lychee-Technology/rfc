...
- `API-specs-authservice.cn.md`：独立的终端用户 token 服务（`cmd/authservice`）—— provider 登录换取、身份绑定、token 刷新/吊销，以及公开 profile 查询。
...
2. [Auth Service（终端用户 Token 服务）APIs](API-specs-authservice.cn.md)
3. [Auth Admin Routes APIs](API-specs-control-plane-service-auth-routes.cn.md)
4. [Control Plane APIs](API-specs-control-plane.cn.md)
