...
To meet enterprise requirements for data sovereignty, privacy, and security auditing, LTBase provides a self-hosted deployment solution based on an "Artifact Subscription" model.
...
The solution uses a pull-based architecture: the Customer's infrastructure requests updates from LTBase, so no external entity has write access to the customer's environment.
...
The design follows Zero Trust and Least Privilege principles.
...
* **No Inbound Access:** LTBase requires no inbound access (Write/AssumeRole) to the customer's AWS environment.
...
To avoid managing long-term AWS Access Keys (AK/SK), the recommended approach is a Dual-Link OIDC (OpenID Connect) trust model.
...
To reduce the risk of service disruption during updates, the LTBase CDK solution uses AWS CodeDeploy together with AWS Lambda aliases. This gives zero-downtime deployments and automated failure recovery.

### B.1 Core mechanism: alias traffic shifting

Instead of replacing the running code instantaneously, CodeDeploy manages a weighted alias strategy.

* **The Alias:** A stable identifier (e.g., Prod) that client applications invoke.  
* **The Shift:** CodeDeploy adjusts the routing weights of this alias, splitting incoming traffic between the current version (N) and the new version (N+1).
...
3. **Continuous Monitoring:**  
   * During the shifting window, CodeDeploy monitors specific CloudWatch alarms (configured by the CDK).  
...
5. **Outcome B: Automatic Rollback**  
   ...
   * **Result:** The system returns to a known good state within seconds, which limits the impact on end-users.
...
LTBase distributes optimized Go binaries as Lambda Zip archives. This approach minimizes cold-start latency and removes the operational overhead and cost of maintaining private Container Registries (ECR).

**Appendix D: Note on Access Control Granularity**

LTBase authorizes the Customer's AWS Account ID (Root) on the bucket policy. This delegates access control to the Customer's internal IAM administrators. They decide which internal roles (e.g., CI/CD pipelines) are permitted to pull artifacts, and internal rotation or reconfiguration does not require LTBase intervention.