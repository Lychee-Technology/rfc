...
- It is read from JWT claims by `permissionsFromRequest` (`internal/request_identity.go`) and used by authorizers such as the control-plane admin check (`controlplane.admin`).
...
| `role_permission_attachment` (permission missing) | `role_permission_attachment` — preserved as-is in canonical SK format (warning emitted for missing permission) |
...
| `resource_grant` (unsupported shape) | Preserved as-is with a warning (`unsupported_grant_selector`) |
...
New `POST /api/v1/auth/policies` and `PUT /api/v1/auth/principals/{type}/{id}/policies/{policy_id}` operate on the canonical model. They do **not** create `permission_profile` or `role_permission_attachment` records.
