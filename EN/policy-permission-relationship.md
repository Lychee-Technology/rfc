...
A policy exists independently. It does not require a permission to exist, and it does not reference permission records.
...
Legacy data (`permissions`, `role_permissions`, `grants`) is relegated to the `legacy` sub-object for diagnostics only. It must **not** appear as primary REST resource fields.
